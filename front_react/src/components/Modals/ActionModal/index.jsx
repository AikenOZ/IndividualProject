...
const ActionModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleFinalSubmit = () => {
    // Вызываем onSubmit с полными данными
      ...
      <div className="flex flex-col items-center space-y-3">
        {Object.entries(USER_CATEGORIES).map(([catKey, catData]) => (
          <button
            ...
            className={`px-4 py-3 text-white font-medium bg-[#2B2B2B] rounded-lg hover:bg-[#323232] transition-all duration-300 w-full max-w-sm text-left ${
              completedCategories[catKey] ? 'ring-2 ring-[#FF4D00]' : ''
            }`}
          >
